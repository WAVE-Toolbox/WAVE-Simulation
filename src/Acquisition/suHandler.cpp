...
void KITGPI::Acquisition::suHandler<ValueType>::readDataSU(std::string const &filename, lama::Matrix<ValueType> &data, IndexType ns, IndexType ntr)
{
    Segy tr;
 ...
 */
template <typename ValueType>
void KITGPI::Acquisition::suHandler<ValueType>::readSingleDataSU(std::string const &filename, lama::Vector<ValueType> &data, IndexType traceNumber)
{
    Segy tr;
 ...
 */
template <typename ValueType>
void KITGPI::Acquisition::suHandler<ValueType>::writeSU(std::string const &filename, lama::DenseMatrix<ValueType> const &data, lama::DenseVector<IndexType> const &coordinates, ValueType DT, IndexType sourceCoordinate, IndexType NX, IndexType NY, IndexType NZ, ValueType DH) const
{
    Segy tr;
    initSegy(tr);
    ...
    IndexType ns = data.getNumColumns();
    IndexType ntr = data.getNumRows();
...
void KITGPI::Acquisition::suHandler<ValueType>::buildAcqMatrix(std::string const &filename, ValueType DH, buildAcqMatPtr buildAcqMat)
{
    ...
    std::vector<lama::DenseMatrix<ValueType>> acqMatVec(4);

    std::string filenameTmp;

    // read all source files
    filenameTmp = filename + ".p.SU";
    (this->*buildAcqMat)(filenameTmp, acqMatVec[0], DH);
    nShotsP = acqMatVec[0].getNumRows();

    filenameTmp = filename + ".vx.SU";
    (this->*buildAcqMat)(filenameTmp, acqMatVec[1], DH);
    nShotsVX = acqMatVec[1].getNumRows();

    filenameTmp = filename + ".vy.SU";
    (this->*buildAcqMat)(filenameTmp, acqMatVec[2], DH);
    nShotsVY = acqMatVec[2].getNumRows();

    filenameTmp = filename + ".vz.SU";
    (this->*buildAcqMat)(filenameTmp, acqMatVec[3], DH);
    nShotsVZ = acqMatVec[3].getNumRows();
...
void KITGPI::Acquisition::suHandler<ValueType>::locateTrace(std::string &filename, scai::IndexType &traceNumber, scai::IndexType shotNumber)
{
    ...
    if (shotNumber < 0) {
        filename += ".p.SU";
        traceNumber = nShotsP + shotNumber;
        return;
    }
    shotNumber -= nShotsVX;
    if (shotNumber < 0) {
        filename += ".vx.SU";
        traceNumber = nShotsVX + shotNumber;
        return;
    }
    shotNumber -= nShotsVY;
    if (shotNumber < 0) {
        filename += ".vy.SU";
        traceNumber = nShotsVY + shotNumber;
        return;
    }
    shotNumber -= nShotsVZ;
    if (shotNumber < 0) {
        filename += ".vz.SU";
        traceNumber = nShotsVZ + shotNumber;
        return;
    }
}
...
scai::IndexType KITGPI::Acquisition::suHandler<ValueType>::getComponentFromName(std::string const &filename)
{
    ...
    IndexType component;
    if (tmpString.compare("p") == 0)
        component = 1;
    else if (tmpString.compare("vx") == 0)
        component = 2;
    else if (tmpString.compare("vy") == 0)
        component = 3;
    else if (tmpString.compare("vz") == 0)
        component = 4;
    else
        COMMON_THROWEXCEPTION("no component found in filename");
 ...
 */
template <typename ValueType>
void KITGPI::Acquisition::suHandler<ValueType>::initSegy(Segy &tr) const
{
