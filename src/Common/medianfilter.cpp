...
void _medianfilter(const element* signal, element* result, int NX, int ksize)
{
...
//     result - output signal
//     NX      - length of the signal
void medianfilter(element* signal, element* result, int NX, int ksize)
{
   //   Check arguments
   if (!signal || NX < 1)
      return;
   ...
   _medianfilter(extension, result ? result : signal, NX + 4, ksize);
   //   Free memory
   delete[] extension;
}
