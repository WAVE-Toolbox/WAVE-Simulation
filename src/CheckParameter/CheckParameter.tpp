...
void KITGPI::CheckParameter::checkNumericalArtefeactsAndInstabilities(const KITGPI::Configuration::Configuration &config, Modelparameter::Modelparameter<ValueType> &model, scai::dmemo::CommunicatorPtr comm)
{
    if (!config.get<bool>("initSourcesFromSU")) {
        ...
        KITGPI::CheckParameter::checkStabilityCriterion<ValueType>(config.get<ValueType>("DT"), config.get<ValueType>("DH"), vMaxTmp, config.get<std::string>("dimension"), config.get<IndexType>("spatialFDorder"), comm);
        KITGPI::CheckParameter::checkNumericalDispersion<ValueType>(config.get<ValueType>("DH"), vMinTmp, fcMax, config.get<IndexType>("spatialFDorder"), comm);
    }
}
...
template <typename ValueType>
void KITGPI::CheckParameter::checkSources(Configuration::Configuration const &config, Acquisition::Sources<ValueType> const &sources, scai::dmemo::CommunicatorPtr comm)
{
...
template <typename ValueType>
void KITGPI::CheckParameter::checkReceivers(Configuration::Configuration const &config, Acquisition::Receivers<ValueType> const &receiver, scai::dmemo::CommunicatorPtr comm)
{
    IndexType NX = config.get<IndexType>("NX");
    IndexType NY = config.get<IndexType>("NY");
    IndexType NZ = config.get<IndexType>("NZ");
